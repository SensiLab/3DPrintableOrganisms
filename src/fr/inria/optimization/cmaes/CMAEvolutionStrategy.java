...
//    	System.out.println("CMAEvolutionStrtegy: constructor called");
        state = -1;
    }
     ...
     * */
    public double[] init() {
//    	System.out.println("CMAEvolutionStrtegy: init called");
    	int i;
    	if (N <= 0)
...
//    	System.out.println("Asserting xmean");
    	assert xmean != null;
    	assert sigma > 0; 
...
/** search space dimensions must be set before the optimization is started. */
    ...
    public String getDataRowFitness() {
        ...
        if (mode == SINGLE_MODE)
            ...
            s += fit.raw[0].val + ","; 					// Best fitness
            s += fit.raw[sp.getLambda()/2].val + ",";	// Median Fitness
            s += fit.raw[sp.getLambda()-1].val + ",";	// Worst Fitness
            s += math.min(diagD) + ","  
    ...
    public void writeToDefaultFiles(String fileNamePrefix) {
        ...
        long firsttime = System.currentTimeMillis();
        writeToFile(fileNamePrefix + "fit.csv", getDataRowFitness(), 1);
        writeToFile(fileNamePrefix + "xmean.dat", getDataRowXMean(), 1);
        writeToFile(fileNamePrefix + "xrecentbest.csv", getDataRowXRecentBest(), 1);
...
//                + (System.currentTimeMillis()-timings.start)  + " " + opts.maxTimeFractionForWriteToDefaultFiles);
    ...
    public void writeToDefaultFilesHeaders(String fileNamePrefix, int flgAppend) {
        ...
        writeToFile(fileNamePrefix + "fit.csv", 
        		s + "iteration, evaluations, sigma, axisratio, bestever_fitness, best_fitness, median_fitness, worst_fitness, mindii, "
        ...
        if (state == 0)
            writeToFile(fileNamePrefix + "xmean.dat", getDataRowXMean(), 1);
        writeToFile(fileNamePrefix + "xrecentbest.csv", 
        s + "iteration, evaluations, sigma, void, fitness_of_recent_best, x_of_recent_best(1...dimension) \n" 
        		, flgAppend);
